...
training_epochs = 400 # total epochs for training
batches_per_epoch = 1000 # steps per epoch
stage1_epochs = 100 # epochs for stage 1
stage3_epochs = 200 # epochs for stage 3
stage1_batch_size = 256 # batch size for training, only used in stage 1
vst_base_batch_size = 160  # It can be adjusted based on the GPU memory used
##########################################################################################
env_params = {
...
logger_params = {
    'log_file': {
        'desc': f"icam_atsp{min_problem_size}_to_{max_problem_size}_epoch{training_epochs}",
        'filename': 'run_log.txt',
        'filepath': './result_atsp_models/' + process_start_time.strftime("%Y%m%d_%H%M%S") + '{desc}'
    }
}
